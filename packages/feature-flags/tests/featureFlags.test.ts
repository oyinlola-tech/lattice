...
const flags: readonly FeatureFlag[] = [
  { key: "dark-mode", enabled: true, defaultValue: false },
  {
    ...
    enabled: true,
    defaultValue: false,
    rules: [{ type: "user", users: ["u1", "u2"], value: true }],
  },
  {
    key: "ai-search",
    enabled: true,
...
describe("createFeatureFlags", () => {
  ...
  it("isEnabled returns true for matched rule", async () => {
    ...
    expect(await flagsApi.isEnabled("new-checkout", { userId: "u1" })).toBe(
      true,
    );
  });
  ...
  it("isEnabled returns false for unmatched rule", async () => {
    ...
    expect(await flagsApi.isEnabled("new-checkout", { userId: "u99" })).toBe(
      false,
    );
  });
  ...
  it("throws on missing flag when configured", async () => {
    ...
    await expect(flagsApi.isEnabled("nonexistent")).rejects.toThrow(
      "nonexistent",
    );
  });
  ...
  it("attribute targeting works", async () => {
    ...
    expect(
      await flagsApi.isEnabled("ai-search", { attributes: { plan: "pro" } }),
    ).toBe(true);
    expect(
      await flagsApi.isEnabled("ai-search", { attributes: { plan: "free" } }),
    ).toBe(false);
  });
  ...
  it("percentage rollout with 100% enables for all", async () => {
    ...
    expect(await flagsApi.isEnabled("rollout", { userId: "anyone" })).toBe(
      true,
    );
  });
  ...
  it("snapshot returns client-visible flags", async () => {
    ...
    const flagsApi = createFeatureFlags({
      provider: createMemoryProvider(clientFlags),
    });
