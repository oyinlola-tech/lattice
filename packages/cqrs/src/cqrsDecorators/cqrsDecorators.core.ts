 ...
 */
const CQRS_HANDLER_METADATA =
  Symbol.for(
    "lattice.cqrs.handler",
  );

/**
 * Metadata key used to identify command handler configuration.
 */
const COMMAND_HANDLER_METADATA =
  Symbol.for(
    "lattice.cqrs.command-handler",
  );

/**
 * Metadata key used to identify query handler configuration.
 */
const QUERY_HANDLER_METADATA =
  Symbol.for(
    "lattice.cqrs.query-handler",
  );

/**
 * Metadata key used to identify command/query type information.
 */
const CQRS_TYPE_METADATA =
  Symbol.for(
    "lattice.cqrs.type",
  );
 ...
 */
export type DecoratedCqrsClass<
  TInstance = object,
> = CqrsClass<TInstance> & {
...
>(
  ...
  return (
    ...
    const constructor =
      target as unknown as DecoratedCqrsClass;

    const metadata: CqrsHandlerMetadata =
      Object.freeze({
        kind,
        type,
      });
 ...
 */
export function createCommandHandlerDecorator(
  type: string,
): ClassDecorator {
 ...
 */
export function createQueryHandlerDecorator(
  type: string,
): ClassDecorator {
  return QueryHandler(
    type,
 ...
 */
function validateHandlerMetadata(
  kind: CqrsHandlerKind,
  type: string,
): void {
  if (
    ...
    throw new TypeError(
      `${kind} handler type cannot be empty.`,
    );
  }
}