...
export class DatabaseClient implements DatabaseClientContract<DatabaseTransactionContext> {
  ...
  public getPrisma(): PrismaClientLike { return this.prisma; }
  ...
  public async connect(): Promise<void> {
    ...
    try {
      ...
      this.status = "error";
      const normalized = this.normalizeError(error, "Database connection failed.");
  ...
  public async disconnect(): Promise<void> {
    ...
    try {
      ...
      this.status = "error";
      const normalized = this.normalizeError(error, "Database disconnection failed.");
      this.logger.error(normalized.message, normalized);
      throw normalized;
    }
  }
  ...
  public getStatus(): DatabaseStatus { return this.status; }
  public getConnectedAt(): Date | undefined { return this.connectedAt; }

  public async healthCheck(): Promise<DatabaseHealth> {
    const startedAt = Date.now();
    try {
      await this.ping();
      return { status: "connected", latencyMs: Date.now() - startedAt, checkedAt: new Date() };
    } catch (error) {
      const normalized = this.normalizeError(error, "Database health check failed.");
      return { status: "error", latencyMs: Date.now() - startedAt, checkedAt: new Date(), error: normalized.message };
    }
  }
  ...
  public async transaction<TResult>(
    ...
    try {
      ...
      const normalized = this.normalizeError(error, "Database transaction failed.");
      this.logger.error(normalized.message, normalized);
      throw normalized;
    }
  }
  ...
  public async queryRaw<TResult = unknown>(query: Prisma.Sql): Promise<TResult> {
    await this.ensureConnected();
    try {
  ...
  public async destroy(): Promise<void> { await this.disconnect(); }
  ...
  private registerQueryLogging(): void {
    ...
    prisma.$on?.("query", (event) => {
      this.logger.debug("Database query executed.", { durationMs: event.duration, target: event.target });
    });
  }
  ...
  private buildTransactionOptions(options: TransactionOptions) {
    return {
      ...
      isolationLevel: options.isolationLevel ? this.mapIsolationLevel(options.isolationLevel) : undefined,
    };
  }

  private mapIsolationLevel(level: NonNullable<TransactionOptions["isolationLevel"]>): Prisma.TransactionIsolationLevel {
    switch (level) {
      case "ReadUncommitted": return Prisma.TransactionIsolationLevel.ReadUncommitted;
  ...
  private normalizeError(error: unknown, fallbackMessage: string): DatabaseError {
    if (error instanceof DatabaseError) return error;
    return new DatabaseError(error instanceof Error ? error.message : fallbackMessage, { cause: error });
  }
}
