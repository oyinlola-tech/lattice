...
export async function lockRow(
  ...
  try {
    ...
    throw new DatabaseError(
      `Failed to acquire row lock on "${tableName}".`,
      {
        cause: error,
        metadata: {
          tableName,
...
function getLockModeSql(
  ...
  switch (mode) {
    ...
    default:
      throw new TypeError(
        `Unsupported database lock mode: ${String(
          mode,
        )}`,
      );
  }
 ...
 */
export function normalizeAdvisoryKey(
  lockKey: string,
): bigint {
  validateLockKey(
    lockKey,
...
function validateLockKey(
  ...
  if (
    ...
    throw new TypeError(
      "A non-empty database lock key is required.",
    );
  }
  ...
  if (
    ...
    throw new TypeError(
      "Database lock keys cannot exceed 255 characters.",
    );
  }
}
...
function validateId(
  ...
  if (
    ...
    throw new TypeError(
      "A non-empty database identifier is required.",
    );
  }
  ...
  if (
    ...
    throw new TypeError(
      "A finite database identifier is required.",
    );
  }
}