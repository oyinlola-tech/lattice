...
describe("PluginRegistryImpl", () => {
  it("registers and retrieves plugins", () => {
    ...
    const plugin = {
      metadata: { name: "@lattice/test" },
    };

    registry.register(plugin);
    expect(registry.has("@lattice/test")).toBe(true);
    expect(registry.get("@lattice/test")?.plugin).toBe(plugin);
  });
  ...
  it("removes registered plugins", () => {
    const registry = new PluginRegistryImpl();
    registry.register({ metadata: { name: "@lattice/test" } });

    expect(registry.remove("@lattice/test")).toBe(true);
    expect(registry.has("@lattice/test")).toBe(false);
  });
});
...
describe("LifecycleController", () => {
  it("transitions through install lifecycle", async () => {
    ...
    registry.register(plugin);
    const registered = registry.get("@lattice/test")!;
...
describe("PluginManager", () => {
  it("registers and lists plugins", async () => {
    ...
    expect(manager.has("@lattice/test")).toBe(true);
  ...
  it("resolves dependencies and starts plugins in order", async () => {
    ...
    manager.register({
      metadata: { name: "@lattice/a" },
      dependencies: [{ name: "@lattice/b" }],
      async initialize() {
        order.push("a");
      },
      async start() {
    ...
    const context = createPluginContext({ metadata: { name: "@lattice/test" } });
...
describe("PluginContext", () => {
  it("creates context with plugin metadata", () => {
    const context = createPluginContext({ name: "@lattice/test" });
    expect(context.plugin.name).toBe("@lattice/test");
...
describe("PluginEvents", () => {
  it("creates plugin lifecycle event", () => {
    ...
    expect(event.plugin.name).toBe("@lattice/test");
...
describe("PluginDiagnostics", () => {
  ...
  it("builds diagnostic report", () => {
    const report = buildDiagnosticReport([
      {
        plugin: { metadata: { name: "@lattice/a" } },
        state: "started" as const,
      },
      {
        plugin: { metadata: { name: "@lattice/b" } },
  ...
  it("includes dependencies in diagnostic", () => {
    const report = buildDiagnosticReport([
      {
        plugin: {
          metadata: { name: "@lattice/a" },
          dependencies: [{ name: "@lattice/b" }],
          optionalDependencies: [{ name: "@lattice/c" }],
        },
        state: "started" as const,
      },
    ]);
    ...
    expect(diagnostic.dependencies).toEqual(["@lattice/b"]);
    expect(diagnostic.optionalDependencies).toEqual(["@lattice/c"]);
  });
});
