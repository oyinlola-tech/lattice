...
/**
 * Validates an OpenAPI document for structural and semantic issues.
 */
...
/**
 * Default OpenAPI validator implementation.
 */
export class OpenAPIValidatorImpl implements OpenAPIValidator {
  public validate(document: OpenAPIDocument): {
    readonly valid: boolean;
    readonly errors: readonly OpenAPIValidationIssue[];
    readonly warnings: readonly OpenAPIValidationIssue[];
  } {
    ...
    this.validateComponents(document, errors, warnings);
  ...
  public assertValid(document: OpenAPIDocument): void {
    ...
    if (!result.valid) {
      throw new OpenAPIValidationError(result.errors);
    }
  }

  private validateDocumentStructure(
    document: OpenAPIDocument,
    errors: OpenAPIValidationIssue[],
    warnings: OpenAPIValidationIssue[],
  ): void {
    if (!document.openapi) {
      errors.push({
        path: "openapi",
    ...
    for (const [path, pathItem] of Object.entries(document.paths)) {
      ...
      for (const method of OPERATIONS) {
        const operation = (pathItem as Record<string, unknown>)[method] as
          | { parameters?: readonly { in?: string; required?: boolean; name?: string }[] }
          | undefined;
  ...
  private validateComponents(
    ...
    // Component validation can be extended here
  }
