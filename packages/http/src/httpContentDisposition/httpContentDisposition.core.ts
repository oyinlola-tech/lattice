...
export const CONTENT_DISPOSITION_HEADER =
  "Content-Disposition";

export const DEFAULT_DISPOSITION_TYPE =
  "inline";
...
export function formatContentDisposition(
  ...
  if (
    options.filenameStar !==
      undefined
  ) {
    parts.push(
      `filename*=${formatRFC5987Value(
        options.filenameStar,
      )}`,
    );
  }
...
export function isValidDispositionType(
  value: string,
): boolean {
  const normalized =
    value
...
> {
  ...
  for (
    const parameter of disposition.parameters
  ) {
    result[
      parameter.name
    ] = parameter.value;
...
export function decodeRFC5987Value(
  value: string,
): string | undefined {
  const trimmed =
    value.trim();
...
export function escapeQuotedString(
  value: string,
): string {
  return value
    .replace(
...
export function createAttachmentDisposition(
  filename: string,
): string {
  const sanitized =
    sanitizeFilename(
...
export function createInlineDisposition(
  filename:
    | string
    | undefined,
): string {
...
export function validateContentDisposition(
  ...
  for (
    const parameter of parsed.parameters
  ) {
    const name =
      parameter.name
        .trim()
...
function normalizeDispositionType(
  value: string,
): string {
  const normalized =
    value
  ...
  if (
    ...
    throw new TypeError(
      "Content-Disposition type cannot be empty.",
    );
  }
...
function normalizeParameterName(
  ...
  if (
    ...
    throw new TypeError(
      "Content-Disposition parameter name cannot be empty.",
    );
  }
  ...
  if (
    ...
    throw new TypeError(
      `Invalid Content-Disposition parameter name: ${value}`,
    );
  }
