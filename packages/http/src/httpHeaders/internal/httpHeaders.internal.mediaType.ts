 ...
 */
export function parseMediaType(value: string): ParsedMediaType | undefined {
  ...
  const type = media.slice(0, separator).trim();

  const subtype = media.slice(separator + 1).trim();
