...
export function mediaTypeSpecificity(
  accepted: ParsedMediaType,
  candidate: ParsedMediaType,
): number {
  if (accepted.type !== "*" && accepted.type !== candidate.type) {
    return 0;
  }

  if (accepted.subtype === "*") {
    return accepted.type === "*" ? 1 : 2;
  }

  if (accepted.subtype.startsWith("*+")) {
    return candidate.subtype.endsWith(accepted.subtype.slice(1)) ? 2 : 0;
  }

  return accepted.subtype === candidate.subtype ? 3 : 0;
}
...
export function languageSpecificity(
  ...
  if (
    candidate.startsWith(`${accepted}-`) ||
    accepted.startsWith(`${candidate}-`)
  ) {
    return 2;
  }
