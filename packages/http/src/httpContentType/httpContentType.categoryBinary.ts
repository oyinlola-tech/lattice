...
import { getMediaType } from "./httpContentType.normalizer.js";
import { matchesContentType } from "./httpContentType.matcher.js";

export function isFormURLEncoded(
  value: string | ContentType | undefined | null,
): boolean {
  return matchesContentType(value, MEDIA_TYPES.FORM_URLENCODED);
}
...
export function isMultipartFormData(
  ...
  return matchesContentType(value, MEDIA_TYPES.MULTIPART_FORM_DATA);
}
...
export function isBinary(
  ...
  return (
    mediaType === MEDIA_TYPES.OCTET_STREAM ||
    mediaType === MEDIA_TYPES.PDF ||
    mediaType === MEDIA_TYPES.ZIP ||
    mediaType === MEDIA_TYPES.GZIP ||
