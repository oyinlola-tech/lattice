...
export function isMethodNotAllowed(status: number): boolean {
...
export function isUnprocessableEntity(status: number): boolean {
  return status === STATUS.UNPROCESSABLE_ENTITY;
}

export function isTooManyRequests(status: number): boolean {
  return status === STATUS.TOO_MANY_REQUESTS;
}
...
export function isGatewayTimeout(status: number): boolean {
  return status === STATUS.GATEWAY_TIMEOUT;
}
