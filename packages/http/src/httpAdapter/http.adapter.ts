...
export interface HttpAdapter {
  ...
  readonly capabilities:
    | Readonly<HttpAdapterCapabilities>;
  ...
  normalizeRequest?(
    ...
    | RequestContextInit
    | Promise<RequestContextInit>;

  normalizeResponse?(
    input: unknown,
  ):
    | ResponseContextInit
    | Promise<ResponseContextInit>;
...
export abstract class BaseHttpAdapter
  ...
  async handle(
    ...
    const request =
      await this.createRequestContext(
        input,
      );
  ...
  protected async write(
    ...
    throw new HttpAdapterError(
      ...
      {
        code:
          "HTTP_RESPONSE_WRITER_NOT_IMPLEMENTED",
        adapter:
          this.name,
      },
    );
...
export class GenericHttpAdapter
  ...
  private readonly requestFactory:
    GenericAdapterOptions["requestFactory"];

  private readonly responseFactory:
    | GenericAdapterOptions["responseFactory"];

  private readonly writerFactory:
    GenericAdapterOptions["writerFactory"];

  private readonly responseWriter:
    | GenericAdapterOptions["responseWriter"];
...
export class HttpAdapterRegistry {
  ...
  register(
    ...
    if (
      ...
      throw new TypeError(
        "HTTP adapter name cannot be empty.",
      );
    }
    ...
    if (
      ...
      throw new HttpAdapterError(
        ...
        {
          code:
            "HTTP_ADAPTER_ALREADY_REGISTERED",
          adapter:
            adapter.name,
        },
      );
  ...
  require(
    ...
    if (
      ...
      throw new HttpAdapterError(
        `HTTP adapter "${name}" is not registered.`,
        {
          code:
            "HTTP_ADAPTER_NOT_FOUND",
          adapter:
            name,
        },
      );
...
export function isHttpAdapter(
  ...
  return (
    ...
    typeof candidate.createResponse ===
      "function" &&
    typeof candidate.createWriter ===
      "function" &&
    typeof candidate.handle ===
      "function"
  );
}
...
export function mergeResponseContext(
  ...
  for (
    const cookie of source.cookies
  ) {
    target.setCookie(
      cookie,
    );
