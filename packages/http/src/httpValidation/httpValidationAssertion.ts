...
import { validateURL } from "./httpValidationUrl.js";
...
export function assertValidHeader(name: string, value: string): void {
  ...
  if (!result.valid) {
    throw new TypeError(result.reason ?? "Invalid HTTP header.");
  }
}
...
export function assertValidURL(value: string): void {
  ...
  if (!result.valid) {
    throw new TypeError(result.reason ?? "Invalid URL.");
  }
}

export function assertValidStatusCode(status: number): void {
