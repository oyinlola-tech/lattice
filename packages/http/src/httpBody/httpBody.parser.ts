...
export const DEFAULT_BODY_PARSER_OPTIONS:
  Required<
    Pick<
      BodyParserOptions,
      "limit" | "encoding" | "strictContentType"
    >
  > = {
  limit:
    DEFAULT_BODY_LIMIT,
...
> {
  ...
  if (
    ...
    throw new UnsupportedBodyTypeError(
      contentType,
    );
  }

  const body =
    await parseByFormat<T>(
      request,
      format,
      options,
    );
...
export function isFormRequest(
  request: IncomingMessage,
): boolean {
  return isFormContentType(
    getRequestContentType(
      request,
    ),
  );
...
export interface ParsedBodyRequest
...
export async function parseBodyWithOptions<
  T = unknown,
>(
  request: IncomingMessage,
  options:
...
export type {
  HTTPBodyReaderOptions,
  HTTPBodyParseOptions,
};