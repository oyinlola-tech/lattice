...
export function createValidationComposer<T>(steps: readonly ValidationStep<T>[], options: ValidationComposerOptions = {}): ValidationComposer<T> {
  ...
  return Object.freeze({
    ...
    assert(value: T): T {
      ...
      if (!result.success) throw new ConstraintValidationError(`${name} validation failed.`, result.issues);
      return result.data;
    },
  });
}
...
export function composeSchemas<T>(...schemas: readonly ValidationSchema<T>[]): ValidationSchema<T> {
  if (schemas.length === 0) throw new TypeError("At least one schema is required.");
...
export function constraintStep<T>(constraint: ValidationConstraint<T>): ValidationStep<T> {
  return (value: T): ValidationResult<T> => checkConstraints([constraint], value);
}

/** Creates a validation step from multiple constraints. */
export function constraintsStep<T>(constraints: readonly ValidationConstraint<T>[]): ValidationStep<T> {
  return (value: T): ValidationResult<T> => checkConstraints(constraints, value);
}

/** Combines validators using logical AND semantics. Every validator must succeed. */
export function all<T>(...validators: readonly ValidationStep<T>[]): ValidationStep<T> {
...
export function any<T>(...validators: readonly ValidationStep<T>[]): ValidationStep<T> {
  return (value: T): ValidationResult<T> => {
    const issues: ValidationIssue[] = [];
    for (const validator of validators) {
      const result = validator(value);
    ...
    return failure(issues.length > 0 ? issues : [{ path: [], code: "no_validator_succeeded", message: "No validation rule accepted the value.", received: value }]);
  };
}

/** Runs validators sequentially and returns the first successful result. */
export function first<T>(...validators: readonly ValidationStep<T>[]): ValidationStep<T> {
  return (value: T): ValidationResult<T> => {
...
export function negate<T>(validator: ValidationStep<T>): ValidationStep<T> {
  return (value: T): ValidationResult<T> => {
    const result = validator(value);
    if (result.success) return failure([{ path: [], code: "negated_validation_failed", message: "Value must not satisfy the supplied validation rule.", received: value }]);
    return success(value);
  };
}

/** Makes a validation step optional. Undefined values bypass the validator. */
export function optional<T>(validator: ValidationStep<T>): ValidationStep<T | undefined> {
...
export function nullable<T>(validator: ValidationStep<T>): ValidationStep<T | null> {
...
export function optionalNullable<T>(validator: ValidationStep<T>): ValidationStep<T | null | undefined> {
  return (value: T | null | undefined): ValidationResult<T | null | undefined> => {
...
export function append<T>(composer: ValidationComposer<T>, step: ValidationStep<T>, options: ValidationComposerOptions = {}): ValidationComposer<T> {
  return createValidationComposer([composer.validate, step], { ...options, name: options.name ?? composer.name });
}
...
export function when<T>(predicate: (value: T) => boolean, validator: ValidationStep<T>): ValidationStep<T> {
  return (value: T): ValidationResult<T> => {
...
export function unless<T>(predicate: (value: T) => boolean, validator: ValidationStep<T>): ValidationStep<T> {
  return (value: T): ValidationResult<T> => {
    if (predicate(value)) return success(value);
    return validator(value);
  };
...
export function mapValidated<T, U>(validator: ValidationStep<T>, mapper: (value: T) => U): ValidationStep<T> {
  return (value: T): ValidationResult<T> => {
    const result = validator(value);
    if (!result.success) return result;
    mapper(result.data);
