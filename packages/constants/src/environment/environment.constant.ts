 ...
 */
export function resolveEnvironment(envOverride?: Record<string, string | undefined>): Environment {
  const raw = (envOverride ?? process.env)[NODE_ENV_KEY];
  if (typeof raw === "string" && raw.trim().length > 0) {
    const normalised = raw.trim().toLowerCase();
    if (normalised === "dev" || normalised === "development") return "development";
    if (normalised === "prod" || normalised === "production") return "production";
 ...
 */
export function isProduction(envOverride?: Record<string, string | undefined>): boolean {
 ...
 */
export function isDevelopment(envOverride?: Record<string, string | undefined>): boolean {
 ...
 */
export function isTest(envOverride?: Record<string, string | undefined>): boolean {
