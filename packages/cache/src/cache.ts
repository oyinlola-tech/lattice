/**
 * @lattice/cache — Cache Service
 *
 * High-level cache service that combines adapter, serializer,
 * key builder, tags, invalidation, locking, and metrics
 * into a single cohesive API.
 */

import type {
  CacheAdapter,
  CacheConfig,
  CacheDeleteResult,
  CacheGetResult,
  CacheHealth,
  CacheHealthChecker,
  CacheKeyBuilder,
  CacheOrComputeOptions,
  CacheOrComputeResult,
  CacheSetResult,
  CacheStats,
  CacheStore,
  CacheTag,
} from "./types.js";
import { DEFAULT_TTL_MS } from "./constants.js";
import {
  DefaultKeyBuilder,
  defaultKeyBuilder,
} from "./key-builder.js";
import {
  DefaultCacheStore,
  createCacheStore,
} from "./store.js";
import {
  InMemoryTagStore,
  createTagStore,
} from "./tags.js";
import {
  CacheInvalidationManager,
  createInvalidationManager,
} from "./invalidation.js";
import {
  CacheLockManager,
  createLockManager,
} from "./lock.js";
import {
  InMemoryCacheMetrics,
  createCacheMetrics,
} from "./metrics.js";

/* -------------------------------------------------------------------------- */
/* Cache Service                                                              */
/* -------------------------------------------------------------------------- */

/**
 * High-level cache service that coordinates all cache operations.
 *
 * Provides a unified API for getting, setting, deleting, and
 * invalidating cache entries with support for tags, locking,
 * metrics, and middleware.
 */
export class CacheService implements CacheHealthChecker {
  private readonly store: CacheStore;
  private readonly keyBuilder: CacheKeyBuilder;
  private readonly tagStore: InMemoryTagStore;
  private readonly invalidation: CacheInvalidationManager;
  private readonly lockManager: CacheLockManager;
  private readonly metrics: InMemoryCacheMetrics;
  private readonly defaultTtl: number;
  private readonly enabled: boolean;

  constructor(options: {
    readonly adapter: CacheAdapter;
    readonly config?: CacheConfig;
    readonly keyBuilder?: CacheKeyBuilder;
  }) {
    this.enabled = options.config?.enabled ?? true;
    this.defaultTtl =
      options.config?.defaultTtl ?? DEFAULT_TTL_MS;

    // Create metrics
    this.metrics =
      options.config?.collectStats !== false
        ? createCacheMetrics()
        : (null as unknown as InMemoryCacheMetrics);

    // Create store with metrics
    this.store = createCacheStore({
      adapter: options.adapter,
      metrics: this.metrics,
    });

    // Create key builder
    this.keyBuilder =
      options.keyBuilder ??
      new DefaultKeyBuilder({
        prefix: options.config?.prefix,
        separator: options.config?.separator,
        namespace: options.config?.namespace,
      });

    // Create tag store and invalidation
    this.tagStore = createTagStore();
    this.invalidation = createInvalidationManager({
      adapter: options.adapter,
      tagStore: this.tagStore,
    });

    // Create lock manager
    this.lockManager = createLockManager();
  }

  /* ---- Read ---- */

  async get<TValue = unknown>(
    key: string,
    options?: { readonly namespace?: string },
  ): Promise<CacheGetResult<TValue>> {
    if (!this.enabled) {
      return { hit: false, value: null };
    }

    const fullKey = this.keyBuilder.build(
      key,
      options,
    );
    return this.store.get<TValue>(fullKey);
  }

  /* ---- Write ---- */

  async set<TValue = unknown>(
    key: string,
    value: TValue,
    options?: {
      readonly ttl?: number;
      readonly tags?: readonly CacheTag[];
      readonly namespace?: string;
    },
  ): Promise<CacheSetResult> {
    if (!this.enabled) {
      return {
        success: false,
        key,
        expiresAt: null,
      };
    }

    const fullKey = this.keyBuilder.build(
      key,
      options,
    );

    const result = await this.store.set<TValue>(
      fullKey,
      value,
      {
        ttl: options?.ttl ?? this.defaultTtl,
        ...options,
      },
    );

    // Register tags if provided
    if (options?.tags && options.tags.length > 0) {
      await this.tagStore.add(
        fullKey,
        options.tags,
      );
    }

    return result;
  }

  /* ---- Delete ---- */

  async delete(
    key: string,
    options?: { readonly namespace?: string },
  ): Promise<CacheDeleteResult> {
    if (!this.enabled) {
      return { deleted: false, key };
    }

    const fullKey = this.keyBuilder.build(
      key,
      options,
    );
    return this.store.delete(fullKey);
  }

  /* ---- Has ---- */

  async has(
    key: string,
    options?: { readonly namespace?: string },
  ): Promise<boolean> {
    if (!this.enabled) return false;

    const fullKey = this.keyBuilder.build(
      key,
      options,
    );
    return this.store.has(fullKey);
  }

  /* ---- Clear ---- */

  async clear(options?: {
    readonly namespace?: string;
    readonly pattern?: string;
  }): Promise<{ readonly cleared: number }> {
    if (!this.enabled) {
      return { cleared: 0 };
    }

    return this.store.clear(options);
  }

  /* ---- Compute ---- */

  /**
   * Returns the cached value for `key`, or computes it using `fn`,
   * stores it, and returns it.
   */
  async getOrSet<TValue>(
    key: string,
    fn: () => Promise<TValue>,
    options?: CacheOrComputeOptions,
  ): Promise<CacheOrComputeResult<TValue>> {
    if (!this.enabled) {
      return {
        value: await fn(),
        cached: false,
      };
    }

    if (!options?.forceRefresh) {
      const cached = await this.get<TValue>(
        key,
        options,
      );

      if (cached.hit) {
        return {
          value: cached.value as TValue,
          cached: true,
        };
      }
    }

    const value = await fn();
    await this.set<TValue>(key, value, options);
    return { value, cached: false };
  }

  /* ---- Tags ---- */

  /**
   * Invalidates all cache entries associated with the given tags.
   */
  async invalidateByTag(
    tags: readonly CacheTag[],
  ): Promise<{ readonly cleared: number }> {
    return this.invalidation.invalidateByTag(tags);
  }

  /* ---- Patterns ---- */

  /**
   * Invalidates all cache entries matching the given glob pattern.
   */
  async invalidateByPattern(
    pattern: string,
  ): Promise<{ readonly cleared: number }> {
    return this.invalidation.invalidateByPattern(
      pattern,
    );
  }

  /* ---- Locking ---- */

  /**
   * Executes a function while holding a cache lock.
   */
  async withLock<T>(
    key: string,
    fn: () => Promise<T>,
    options?: {
      readonly ttl?: number;
      readonly retryAttempts?: number;
    },
  ): Promise<T> {
    return this.lockManager.withLock(key, fn, {
      ttl: options?.ttl,
      retry: options?.retryAttempts
        ? {
            attempts: options.retryAttempts,
            delay: 100,
          }
        : undefined,
    });
  }

  /* ---- Stats ---- */

  /** Returns aggregate cache statistics. */
  getStats(): CacheStats | null {
    if (!this.metrics) return null;
    return this.metrics.getStats();
  }

  /* ---- Health ---- */

  async healthCheck(): Promise<CacheHealth> {
    const start = performance.now();

    try {
      // Try a simple operation to verify connectivity
      await this.store.has("__health__");
      const latencyMs = performance.now() - start;

      return {
        healthy: true,
        adapter: this.store.name,
        latencyMs,
        checkedAt: new Date(),
      };
    } catch (error) {
      return {
        healthy: false,
        adapter: this.store.name,
        checkedAt: new Date(),
        error:
          error instanceof Error
            ? error.message
            : String(error),
      };
    }
  }

  /* ---- Lifecycle ---- */

  async connect(): Promise<void> {
    await this.store.connect?.();
  }

  async disconnect(): Promise<void> {
    await this.store.disconnect?.();
  }
}

/* -------------------------------------------------------------------------- */
/* Factory                                                                    */
/* -------------------------------------------------------------------------- */

/**
 * Creates a cache service with the given adapter and config.
 */
export function createCacheService(options: {
  readonly adapter: CacheAdapter;
  readonly config?: CacheConfig;
  readonly keyBuilder?: CacheKeyBuilder;
}): CacheService {
  return new CacheService(options);
}
