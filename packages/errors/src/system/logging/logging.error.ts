 ...
 */
export interface LoggingErrorOptions extends Omit<
  BaseErrorOptions,
  "category"
> {
...
export class LoggingError extends BaseError {
  constructor(message: string, options: LoggingErrorOptions = {}) {
    super(message, {
      ...options,
      code: options.code ?? ErrorCode.LOGGING_FAILED,
      category: options.category ?? ErrorCategory.LOGGING,
 ...
 */
export function isLoggingError(value: unknown): value is LoggingError {
