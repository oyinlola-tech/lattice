...
export class RequestBodyTimeoutError extends HttpError {
  constructor(message = "Request Body Timeout", options: HttpClientErrorOptions = {}) {
    super(message, { ...options, statusCode: 408, code: options.code ?? "REQUEST_BODY_TIMEOUT" });
...
export class UnsupportedProtocolError extends HttpError {
  ...
  constructor(protocol?: string, options: HttpClientErrorOptions = {}) {
    super(
      protocol ? `Unsupported protocol "${protocol}".` : "Unsupported protocol.",
      { ...options, statusCode: 400, code: options.code ?? "UNSUPPORTED_PROTOCOL" },
    );
...
export class InvalidHeaderError extends HttpError {
  ...
  constructor(header?: string, message?: string, options: HttpClientErrorOptions = {}) {
    super(
      message ?? (header ? `Invalid HTTP header "${header}".` : "Invalid HTTP header."),
...
export class InvalidContentTypeError extends HttpError {
  ...
  constructor(contentType?: string, options: HttpClientErrorOptions = {}) {
    super(
      contentType ? `Invalid content type "${contentType}".` : "Invalid content type.",
      { ...options, statusCode: 415, code: options.code ?? "INVALID_CONTENT_TYPE" },
    );
