 ...
 */
export interface ConflictErrorOptions extends Omit<BaseErrorOptions, "category"> {
...
export class ConflictError extends BaseError {
  constructor(message = "The operation conflicts with the current state of the resource.", options: ConflictErrorOptions = {}) {
    super(message, {
      ...options,
...
export function alreadyExistsError(resource: string, identifier?: string | number): ConflictError {
...
export function duplicateError(field: string, value?: string | number): ConflictError {
  ...
  return new ConflictError(message, {
    ...
    statusCode: 409,
    expose: true,
    metadata: { field, ...(value !== undefined ? { value: String(value) } : {}) },
  });
}

/** Creates an error indicating that a resource is currently locked. */
export function resourceLockedError(resource: string, identifier?: string | number): ConflictError {
  const message = identifier !== undefined
