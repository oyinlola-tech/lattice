...
/**
 * Error raised when a client exceeds an allowed request or
 * operation rate.
 */
export class RateLimitError
   ...
   */
  public getRetryAfterMilliseconds():
    | number
    | undefined {
    if (
      this.retryAfterSeconds ===
  ...
  /**
   * Returns a serialized representation including retry information.
   */
...
export function isRateLimitError(
  ...
  return (
    value instanceof RateLimitError
  );
}

/**
 * Creates a rate-limit error that asks the client to retry after
 * the specified number of seconds.
 */
export function retryAfterError(
  retryAfterSeconds: number,
): RateLimitError {
  return new RateLimitError(
    "Too many requests. Please try again later.",
