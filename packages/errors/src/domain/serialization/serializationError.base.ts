...
export function createSerializationError(message: string, options: SerializationErrorOptions = {}): SerializationError {
...
export function toSerializationError(error: unknown, options: { message?: string; code?: string } = {}): SerializationError {
  if (error instanceof SerializationError) {
    return error;
  }
  if (error instanceof Error) {
    return new SerializationError(options.message ?? error.message, {
      code: options.code as ErrorCode | undefined, cause: error,
    });
  }
