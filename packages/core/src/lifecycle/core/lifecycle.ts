...
/**
 * A lifecycle participant can participate in application
...
export interface LifecycleParticipant {
  ...
  /**
   * Releases resources owned by the participant.
   */
...
export interface LifecycleOptions {
  /**
   * Logger used to report lifecycle activity.
   */
  readonly logger?: Logger;

  /**
   * Whether shutdown should continue if one participant fails.
   *
   * Defaults to true.
   */
...
/**
 * Coordinates initialization, startup, shutdown, and disposal
...
export class Lifecycle {
  ...
  private operationLock: Promise<void> | undefined;
  ...
  /**
   * Registers a lifecycle participant.
   *
   * Participants are initialized and started in registration order.
   * They are stopped and disposed in reverse registration order.
   */
  public register(
    ...
    if (
      ...
      throw new Error(
        `Cannot register lifecycle participant "${participant.name}" ` +
          `while application is "${this.state}".`,
      );
    }
  ...
  /**
   * Initializes all registered participants.
   */
  public async initialize(): Promise<void> {
    ...
    try {
      for (const participant of this.participants) {
        this.logger?.debug(
          "Initializing lifecycle participant",
          {
            participant: participant.name,
          },
        );
  ...
  /**
   * Starts all registered participants.
   *
   * If the lifecycle is in CREATED state, it will automatically
   * initialize first. If initialization is already in progress,
   * this method will wait for it to complete.
   */
  public async start(): Promise<void> {
    ...
    try {
      for (const participant of this.participants) {
        this.logger?.debug(
          "Starting lifecycle participant",
          {
            participant: participant.name,
          },
        );
   ...
   *
   * This handles the case where start() is called while
  ...
  /**
   * Stops all registered participants.
   *
   * Participants are stopped in reverse registration order.
   */
  public async stop(): Promise<void> {
    ...
    for (
      let index = this.participants.length - 1;
      index >= 0;
      index -= 1
    ) {
      ...
      try {
        this.logger?.debug(
          "Stopping lifecycle participant",
          {
            participant: participant.name,
          },
        );
        ...
        this.logger?.error(
          "Failed to stop lifecycle participant",
          error,
          {
            participant: participant.name,
          },
  ...
  /**
   * Disposes all registered participants.
   *
  ...
  public async dispose(): Promise<void> {
    ...
    for (
      ...
      try {
        this.logger?.debug(
          "Disposing lifecycle participant",
          {
            participant: participant.name,
          },
        );
        ...
        this.logger?.error(
          "Failed to dispose lifecycle participant",
          error,
          {
            participant: participant.name,
          },
  ...
  /**
   * Returns a snapshot of registered participants.
   */
