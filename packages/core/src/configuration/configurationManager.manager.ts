...
import type { ConfigurationLoaderOptions, ConfigurationLoadResult } from "./loader/configurationLoader.loader.js";
...
export class ConfigurationManager {
  ...
  private stateValue: ConfigurationManagerState = ConfigurationManagerState.CREATED;

  public constructor(options: ConfigurationManagerOptions = {}) {
    this.loader = options.loader ?? new ConfigurationLoader(options.loaderOptions);
    ...
    this.redactor = options.redactor ?? new ConfigurationRedactor(options.redactorOptions);
    this.validationOptions = options.validationOptions ?? {};
  }

  public async initialize(): Promise<ConfigurationManagerResult> {
    if (this.stateValue === ConfigurationManagerState.LOADING) throw new Error("Configuration initialization is already in progress.");
    ...
    try {
      ...
      return { configuration: this.configuration, load: loadResult, validation };
    } catch (error) {
      this.stateValue = ConfigurationManagerState.FAILED;
      throw error;
    }
  ...
  public async reload(): Promise<ConfigurationManagerResult> {
    if (this.stateValue !== ConfigurationManagerState.READY) return this.initialize();
    ...
    try {
      ...
      if (this.configuration !== previousConfiguration) this.configuration = previousConfiguration;
      this.stateValue = previousConfiguration ? ConfigurationManagerState.READY : ConfigurationManagerState.FAILED;
      throw error;
    }
  }

  public getConfiguration(): Configuration { this.ensureReady(); return this.configuration as Configuration; }
  public getProvider(): ConfigurationProvider { return this.provider; }
  public getRegistry(): ConfigurationRegistry { return this.registry; }
  public getSchemaRegistry(): ConfigurationSchemaRegistry { return this.schemas; }
  public getLoader(): ConfigurationLoader { return this.loader; }
  public getRedactor(): ConfigurationRedactor { return this.redactor; }
  public getState(): ConfigurationManagerState { return this.stateValue; }
  public isReady(): boolean { return this.stateValue === ConfigurationManagerState.READY; }
  public getValidationReport(): ConfigurationValidationReport | undefined { return this.validationReport; }
  public getLoadResult(): ConfigurationLoadResult | undefined { return this.loadResult; }
  public getRedactedConfiguration(): RedactedConfiguration | undefined { return this.configuration ? this.redactor.redact(this.configuration) : undefined; }
  public get<T = unknown>(path: string): T | undefined { return this.getProvider().get<T>(path); }
  public require<T = unknown>(path: string): T { return this.getProvider().require<T>(path); }

  private providerConfiguration(configuration: Configuration): void {
    if (this.provider instanceof DefaultConfigurationProvider) this.provider.setConfiguration(configuration);
  }

  private ensureReady(): void {
    if (this.stateValue !== ConfigurationManagerState.READY) throw new Error(`Configuration is not ready. Current state: ${this.stateValue}.`);
  }
}

/** Creates a ConfigurationManager. */
export function createConfigurationManager(options: ConfigurationManagerOptions = {}): ConfigurationManager {
