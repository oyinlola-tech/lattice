...
export class Configuration {
  ...
  public require<T = ConfigurationValue>(path: string): T {
    ...
    if (value === undefined)
      throw new Error(`Required configuration "${path}" is not defined.`);
    return value;
  }
  ...
  public requireByKey<T>(key: ConfigurationKey<T>): T {
    ...
    if (value === undefined)
      throw new Error(`Required configuration "${key.path}" is not defined.`);
    return value;
  }
  ...
  public scope(namespace: string): Configuration {
    ...
    if (!normalizedNamespace)
      throw new Error("Configuration scope cannot be empty.");
    const scopedValues =
  ...
  public toObject(): Readonly<Record<string, ConfigurationValue>> {
    return this.values;
  }

  public merge(other: Configuration): Configuration {
    return new Configuration({
      values: { ...this.values, ...other.toObject() },
      source: other.source,
  ...
  private deleteNestedValue(
    ...
    const result = structuredClone(source) as Record<
      string,
      ConfigurationValue
    >;
    let current: Record<string, ConfigurationValue> = result;
