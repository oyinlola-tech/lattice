...
export class DefaultRuntimeManager implements RuntimeManager {
  ...
  public constructor(
    ...
    this._environment = createRuntimeEnvironment({
      mode: this._options.mode,
      role: this._options.role,
    });
    this._context = createRuntimeContext(
      this._identity,
      {
        ...
        moduleLifecycle: dependencies.moduleLifecycle,
      },
  ...
  public get stopped(): boolean {
    return this.state === RuntimeState.STOPPED;
  }
  public get failed(): boolean {
    return this.state === RuntimeState.FAILED;
  }
  ...
  public async start(): Promise<void> {
    ...
    if (this._state.startPromise) {
      return this._state.startPromise;
    }
    this._state.startPromise = performStart(
      undefined,
      (next, reason) => this.transitionTo(next, reason),
      this._state,
    );
    try {
      await this._state.startPromise;
    } finally {
      this._state.startPromise = undefined;
    }
  }
  ...
  public async stop(): Promise<void> {
    ...
    if (this._state.stopPromise) {
      return this._state.stopPromise;
    }
    this._state.stopPromise = performStop(
      undefined,
      (next, reason) => this.transitionTo(next, reason),
      this._state,
    );
    try {
      ...
      this._state.stopPromise = undefined;
    }
  }
  ...
  public getStateSnapshot(): RuntimeStateSnapshot {
    return createRuntimeStateSnapshot(this.state);
  }
