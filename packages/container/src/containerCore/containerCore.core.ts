...
export class Container {
  ...
  start(): this { this.ensureNotDisposed(); if (this.started) return this; this.started = true; return this; }
  ...
  registerClass<T>(token: RegistrationToken<T>, ctor: new (...args: unknown[]) => T, options: CreateRegistrationOptions = {}): ContainerRegistration<T> {
  ...
  registerValue<T>(token: RegistrationToken<T>, value: T, options: CreateRegistrationOptions = {}): ContainerRegistration<T> {
    return this.register(token, valueProvider(value), { ...options, scope: Scope.SINGLETON });
  }
  ...
  resolve<T>(token: RegistrationToken<T>): T {
    ...
    this.lifecycle.track(result.token, result.value, ContainerLifecycleOwner.CONTAINER);
    return result.value;
  }

  resolveMany<T>(tokens: readonly RegistrationToken<T>[]): T[] { return tokens.map((t) => this.resolve(t)); }
  ...
  canResolve<T>(token: RegistrationToken<T>): boolean { this.ensureNotDisposed(); return this.resolver.canResolve(token); }
  has<T>(token: RegistrationToken<T>): boolean { this.ensureNotDisposed(); return this.registry.has(token); }
  ...
  remove<T>(token: RegistrationToken<T>): boolean { this.ensureMutable(); return this.registry.remove(token); }

  createScope(options: ContainerScopeOptions = {}): ContainerScopeContext {
    this.ensureActive();
    if (!this.options.allowScopes) throw new Error("Container scopes are disabled.");
    return new ContainerScopeContext(this, options);
  }

  getRegistrations(): readonly ContainerRegistration[] { this.ensureNotDisposed(); return this.registry.getAll(); }
  get registrationCount(): number { return this.registry.size; }
  isStarted(): boolean { return this.started; }
  isDisposed(): boolean { return this.disposed; }
  clearSingletons(): void { this.ensureActive(); this.resolver.clearSingletons(); }

  async dispose(): Promise<void> {
    if (this.disposed) return;
    try { if (this.options.autoDispose) await this.lifecycle.disposeContainer(); else this.lifecycle.reset(); }
    finally { this.resolver.clearSingletons(); this.disposed = true; this.started = false; }
  }

  get resolutionOptions(): { autoRegisterClasses: boolean; detectCircularDependencies: boolean; maxResolutionDepth: number } {
    return {
      autoRegisterClasses: this.options.resolution.autoRegisterClasses ?? true,
      detectCircularDependencies: this.options.resolution.detectCircularDependencies ?? true,
      maxResolutionDepth: this.options.resolution.maxResolutionDepth ?? 100,
    };
  }

  private ensureActive(): void { this.ensureNotDisposed(); if (!this.started) this.start(); }
  private ensureNotDisposed(): void { if (this.disposed) throw new Error(`Container "${this.name}" has already been disposed.`); }
  private ensureMutable(): void { this.ensureNotDisposed(); if (!this.options.freezeRegistrations) return; if (this.started) throw new Error(`Registrations for container "${this.name}" are frozen.`); }
}

export function createContainer(options: ContainerOptions = {}): Container { return new Container(options); }
export function createStartedContainer(options: ContainerOptions = {}): Container { return new Container(options).start(); }
