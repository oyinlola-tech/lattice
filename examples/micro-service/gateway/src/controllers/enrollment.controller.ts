 ...
 */
export async function createEnrollment(req: IncomingMessage, res: ServerResponse): Promise<void> {
 ...
 */
export async function listEnrollments(req: IncomingMessage, res: ServerResponse): Promise<void> {
 ...
 */
export async function getEnrollmentById(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const url = new URL(req.url ?? "/", `http://${req.headers.host ?? "localhost"}`);
 ...
 */
export async function updateEnrollmentStatus(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const url = new URL(req.url ?? "/", `http://${req.headers.host ?? "localhost"}`);
  const pathParts = url.pathname.split("/");
  ...
  const result = await client.patch(`/api/v1/enrollments/${enrollmentId}/status`, body, {
    authorization: authHeader,
  });
 ...
 */
export async function withdrawEnrollment(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const url = new URL(req.url ?? "/", `http://${req.headers.host ?? "localhost"}`);
  const pathParts = url.pathname.split("/");
  ...
  const result = await client.post(`/api/v1/enrollments/${enrollmentId}/withdraw`, {}, {
    authorization: authHeader,
  });
