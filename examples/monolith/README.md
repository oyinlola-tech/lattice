# Zudojs Monolith Example

A reference architecture for building modular monoliths with the Zudojs framework.
...
- **CQRS** — Commands write state; queries read state (via `@zudojs/cqrs`)
...
- Node.js ≥ 24
- `@zudojs/cqrs` — Command/query buses
- `@zudojs/events` — Event bus with middleware
- `@zudojs/config` — Layered configuration
- `@zudojs/errors` — Structured error hierarchy
- `@zudojs/validation` — Zod-based validation
...
Copy `.env.example` to `.env` and adjust values. The app uses `@zudojs/config` to resolve sources in priority order:
