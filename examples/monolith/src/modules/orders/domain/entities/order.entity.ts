...
export class Order extends AggregateRoot<OrderId> {
  ...
  public static create(
    ...
    order.addEvent(
      createDomainEvent("order.created", id, {
        userId,
        items: items.map((i) => ({
  ...
  public get totalAmount(): Money {
    return this._items.reduce(
      (t, i) => t.add(i.unitPrice.multiply(i.quantity)),
      Money.create(0),
    );
  }
  ...
  public confirm(): void {
    ...
    this.touch();
    this.addEvent(
      createDomainEvent("order.confirmed", this.id, {
        userId: this._userId,
  ...
  public cancel(): void {
    ...
    this.touch();
    this.addEvent(
      createDomainEvent("order.cancelled", this.id, {
        userId: this._userId,
        previousStatus: prev,
      }),
    );
