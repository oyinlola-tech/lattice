...
import { createUserId, createProductId, createOrderId } from "./shared/domain/ids.js";
...
async function main(): Promise<void> {
  ...
  const { CreateUserCommand } = await import("./modules/users/application/commands/create-user/create-user.command.js");
  const { CreateProductCommand } = await import("./modules/products/application/commands/create-product/create-product.command.js");
  const { CreateOrderCommand } = await import("./modules/orders/application/commands/create-order/create-order.command.js");
  const { GetOrderQuery } = await import("./modules/orders/application/queries/get-order/get-order.query.js");
  const { ListUserOrdersQuery } = await import("./modules/orders/application/queries/list-user-orders/list-user-orders.query.js");
  ...
  await users.getCommandBus().execute(
    new CreateUserCommand(userId, "alice@example.com", "Alice Johnson", "hashed-pw", UserRole.ADMIN),
  );

  await products.getCommandBus().execute(new CreateProductCommand(laptopId, "Laptop Pro 16", "High-performance laptop", 2499.99, "USD", 10));
  await products.getCommandBus().execute(new CreateProductCommand(phoneId, "Phone Ultra", "Latest smartphone", 999.99, "USD", 25));
  await products.getCommandBus().execute(new CreateProductCommand(headphoneId, "Wireless Headphones", "Noise-cancelling", 349.99, "USD", 50));
  ...
  ) as { order: { id: string; status: string; totalAmount: { format(): string }; items: readonly unknown[] }; events: readonly { type: string; aggregateId: string }[] };
  ...
  const queriedOrder = await orders.getQueryBus().execute(
    new GetOrderQuery(orderId),
  ) as { id: string; userId: string; status: string; totalAmount: { format(): string }; items: readonly { productId: string; quantity: number; unitPrice: { amount: number } }[] } | null;
  ...
  if (queriedOrder) {
    ...
    for (const item of queriedOrder.items) {
      console.log(`    - ${item.productId}: ${item.quantity}x $${item.unitPrice.amount}`);
    }
  }
  console.log();

  const userOrders = await orders.getQueryBus().execute(new ListUserOrdersQuery(userId)) as unknown[];
  ...
  console.log("  Domain        → Entities, value objects, events, business rules");
  ...
  console.log("  ✓ Dependency inversion (domain doesn't know about infrastructure)");
