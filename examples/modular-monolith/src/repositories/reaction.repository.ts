...
export interface ReactionRepository {
  ...
  findByUserAndArticle(userId: UserId, articleId: ArticleId): Promise<ReactionModel | null>;
...
export class SqliteReactionRepository implements ReactionRepository {
  public async findByArticle(articleId: ArticleId): Promise<readonly ReactionModel[]> {
    const db = getDatabase();
    const rows = db.prepare("SELECT * FROM reactions WHERE article_id = ?").all(articleId) as Record<string, unknown>[];
    return rows.map(mapRowToReaction);
  }

  public async findByUserAndArticle(userId: UserId, articleId: ArticleId): Promise<ReactionModel | null> {
    const db = getDatabase();
    const row = db.prepare("SELECT * FROM reactions WHERE user_id = ? AND article_id = ?").get(userId, articleId) as Record<string, unknown> | undefined;
  ...
  public async save(reaction: ReactionModel): Promise<void> {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `).run(reaction.id, reaction.articleId, reaction.userId, reaction.type, reaction.createdAt.toISOString());
  }

  public async delete(userId: UserId, articleId: ArticleId): Promise<void> {
    const db = getDatabase();
    db.prepare("DELETE FROM reactions WHERE user_id = ? AND article_id = ?").run(userId, articleId);
  }

  public async countByArticle(articleId: ArticleId): Promise<Record<ReactionType, number>> {
    const db = getDatabase();
    const rows = db.prepare("SELECT type, COUNT(*) as count FROM reactions WHERE article_id = ? GROUP BY type").all(articleId) as { type: string; count: number }[];
