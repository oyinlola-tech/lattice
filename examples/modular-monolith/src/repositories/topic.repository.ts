...
export class SqliteTopicRepository implements TopicRepository {
  ...
  public async findAll(): Promise<readonly TopicModel[]> {
    const db = getDatabase();
    const rows = db.prepare("SELECT * FROM topics ORDER BY follower_count DESC").all() as Record<string, unknown>[];
  ...
  public async save(topic: TopicModel): Promise<void> {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(topic.id, topic.name, topic.description, topic.followerCount, topic.createdAt.toISOString(), topic.updatedAt.toISOString());
  }

  public async incrementFollowerCount(id: TopicId): Promise<void> {
    const db = getDatabase();
    db.prepare("UPDATE topics SET follower_count = follower_count + 1, updated_at = datetime('now') WHERE id = ?").run(id);
  }

  public async decrementFollowerCount(id: TopicId): Promise<void> {
    const db = getDatabase();
    db.prepare("UPDATE topics SET follower_count = MAX(0, follower_count - 1), updated_at = datetime('now') WHERE id = ?").run(id);
  }
}
...
export class SqliteTopicFollowerRepository implements TopicFollowerRepository {
  public async isFollowing(userId: UserId, topicId: TopicId): Promise<boolean> {
    const db = getDatabase();
    const row = db.prepare("SELECT 1 FROM topic_followers WHERE user_id = ? AND topic_id = ?").get(userId, topicId);
    return row !== undefined;
  }

  public async follow(userId: UserId, topicId: TopicId): Promise<void> {
    const db = getDatabase();
    db.prepare("INSERT OR IGNORE INTO topic_followers (topic_id, user_id, created_at) VALUES (?, ?, datetime('now'))").run(topicId, userId);
  }

  public async unfollow(userId: UserId, topicId: TopicId): Promise<void> {
    const db = getDatabase();
    db.prepare("DELETE FROM topic_followers WHERE user_id = ? AND topic_id = ?").run(userId, topicId);
  }
}
