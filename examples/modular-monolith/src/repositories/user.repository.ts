...
export interface UserRepository {
  ...
  update(id: UserId, data: Partial<Pick<UserModel, "name" | "bio" | "avatar">>): Promise<void>;
...
export class SqliteUserRepository implements UserRepository {
  ...
  public async save(user: UserModel): Promise<void> {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(user.id, user.email, user.name, user.bio, user.avatar, user.role, user.createdAt.toISOString(), user.updatedAt.toISOString());
  }

  public async update(id: UserId, data: Partial<Pick<UserModel, "name" | "bio" | "avatar">>): Promise<void> {
    const db = getDatabase();
    const sets: string[] = [];
