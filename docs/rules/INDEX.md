# Zudojs Package Rules

> This document defines the internal development standards for every `@zudojs/*` package.
...
- Do not import files from another package's internal folders (e.g., `@zudojs/events/src/internal/...`).
- Always import from the package's public API (`@zudojs/events`).
...
// ❌ Wrong
import * as Events from "@zudojs/events";
```
...
1. Node.js built-ins
2. External packages
3. `@zudojs/*` packages (alphabetical)
...
import type { EventBus } from "@zudojs/events.js";
...
// ❌ Wrong
import { EventBus } from "@zudojs/events/src/eventBus/eventBus.type.js";
```
...
```ts
// src/index.ts
export type { EventBus } from "@zudojs/events.js";
```
...
- Tier 0 packages must not depend on any `@zudojs/*` package.
...
All `@zudojs/*` dependencies must use exact versions (`0.1.0`).
...
### 10.1 Use @zudojs/errors

All error types must live in `@zudojs/errors`.
No package should create its own error classes that extend `Error` or `BaseError`.

### 10.2 Domain Errors

Each package may define lightweight wrapper classes for package-specific context, but the base error must come from `@zudojs/errors`.
...
Test utilities must live in `@zudojs/testing`.
