# Contributing to Zudojs

> Thank you for your interest in contributing to Zudojs.
...
```bash
git clone https://github.com/zudojs-framework/zudojs.git
cd zudojs
```
...
4. Run checks:
   ...
   npm run typecheck
   npm run architect:check
   npm run test --workspace=@zudojs/<package-name>
   ```
...
1. Identify which existing `@zudojs/*` packages the new package needs.
...
When adding a dependency on another `@zudojs/*` package:
...
npm run test --workspace=@zudojs/events

# Run tests in watch mode
npm run test:watch --workspace=@zudojs/events
```
...
All errors must extend `BaseError` from `@zudojs/errors`.
...
All external input must be validated at the boundary using `@zudojs/security`.
...
- **Issues:** https://github.com/zudojs-framework/zudojs/issues
- **Discussions:** https://github.com/zudojs-framework/zudojs/discussions
